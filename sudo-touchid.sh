...
touch_pam='auth       sufficient     pam_wtid.so'
sudo_path='/etc/pam.d/sudo'

pam_wtid_path='/usr/local/lib/pam/pam_wtid.so.2'
pam_wtid_build_path="/tmp/pam_wtid"
pam_wtid_repo_url="https://raw.githubusercontent.com/inickt/pam_wtid/main"
...
build_pam_wtid() {
    # Download patch
    curl -O --create-dirs --output-dir $pam_wtid_build_path $pam_wtid_repo_url/patch.py
    curl -O --create-dirs --output-dir $pam_wtid_build_path $pam_wtid_repo_url/Makefile
    # Install patch
    (cd $pam_wtid_build_path && make install)
}
